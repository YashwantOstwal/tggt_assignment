        ...
        <div className = 'w-[70%] h-[80%] max-md:h-[50%] max-md:w-[90%]' >
            <div className = 'grid md:grid-cols-2 max-md:grid-rows-2 h-full bg-[#141414] rounded-lg border-[#33C8FE]/80 border-t overflow-hidden'>
                ...
                <div className = 'flex flex-col justify-center'>
                    <Stats title = "Stats/Metrics" clusterCloudUpto4 = {[`Health points: ${pokemon.stats[0].base_stat}`,`Attack: ${pokemon.stats[1].base_stat}`,`Defense: ${pokemon.stats[2].base_stat}`,`Speed: ${pokemon.stats[5].base_stat}`]} row={1}/>
                    <Stats title = "Data" clusterCloudUpto4 = {[`Base experience: ${pokemon.base_experience}`,`Weight: ${pokemon.weight}`,`Height: ${pokemon.height}`,`Order: ${pokemon.order}`]} row={0} />
                    <Stats title = "Specials" clusterCloudUpto4 = {[,`Special-attack: ${pokemon.stats[3].base_stat}`,`Special-defense: ${pokemon.stats[4].base_stat}`]} row={2}/>
                </div>
            </div>        
        </div>
    </div>
